...
var win_1 = floaty.rawWindow(
    <frame id="logo" w="44" h="44" alpha="0.8" marginTop ="0">//悬浮按钮
        <img w="44" h="44" src="#ffffff" circle="true" alpha="0" />
...
var win_2 = floaty.rawWindow(
    <frame id="logo" w="{{device.width}}px" h="44" alpha="0">//悬浮按钮 弹性替身
        <img w="44" h="44" circle="true" alpha="0" />
...
context.startActivity(intent);//切换到前台
    img_down();
})
  ...
  //  log(event.getAction())
    switch (event.getAction()){
        ...
        case event.ACTION_MOVE:
             if(new Date()-downTime > 2500){//拖动关闭脚本
                toastLog("关闭程序")
...
var w = floaty.rawWindow(
    ...
    w.off.setVisibility(8)
      ...
      // threads.start(function () {
      //   setInterval(() => {
      //     log("内存剩余"+(device.getAvailMem() / 1024 / 1024).toFixed(0) +"M");
      //   }, 3000);
      // });
  ...
  ui.run(function(){
    ui.inflate( 
      ...
      <pref-checkbox id= "rebot" text = "游戏自动重启间隔" />
      <pref-input id = "rebothour" inputType = "number"/>
      <text text = "小时" textColor = "black"/>
      </horizontal> */}
      <pref-checkbox id= "stuckorloseflag" text = "卡死/失败检测" layout_weight = "1"/>
            ...
            function(){
              ...
              <vertical margin = "5" id = "v23"> 
                ...
                <horizontal>
                  ...
                  <pref-checkbox id="usediamond5"  text="碎石头)"  />
                  </horizontal>
  ...
  ui.run(function(){
      ui.inflate(
        <vertical margin = "5" id = "v3"> 
          ...
          <text color = "black">(</text>
          <pref-checkbox id="usediamond8"  text="碎石头)"  />
          </horizontal> 
    ...
    function(){
      ui.inflate(
        <vertical margin = "5" id = "v4"> 
          ...
          <text color = "black">(</text>
          <pref-checkbox id="usediamond9"  text="碎石头)"  />
          </horizontal>     
          <horizontal>
          <text text="普通图倍速：" marginLeft = "6" textColor="black" layout_gravity = "center"/>
  ...
  w.viewpager.addOnPageChangeListener(new ViewPager.OnPageChangeListener({
    ...
    // ,onPageScrolled:function(willdisplayposi){
    //   // log(beginposi)
    // },
    // onPageScrollStateChanged:function(state){//0 静止 1拖拽中 2 自动靠向最近页面中
...
threads.start(function(){
    var isHorizontal = isHorizontalScreen();//当前屏幕方向
...
setInterval(() => {
    if (isHorizontal != isHorizontalScreen()) {
        ...
        if(isHorizontal){
            ...
            win_1.setPosition(0,50)//悬浮按钮定位
        }else{
            w.setPosition(50,device.height/4)
          win_1.setPosition(0,0)//悬浮按钮定位
        }
    }
}, 1000)
