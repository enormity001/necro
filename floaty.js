...
var friendpool = storage.get("friendpool",false);
var metro = storage.get("metro",false);
var dailyraid = storage.get("dailyraid",false);
var chips = storage.get("chips",false);
var hellbattle = storage.get("hellbattle",false);
var noconsumeap = storage.get("noconsumeap",false);
var dailyarr = [friendpool,metro,dailyraid,[chips,hellbattle,noconsumeap]]
...
var win_1 = floaty.rawWindow(
    <frame id="logo" w="44" h="44" alpha="0.4" marginTop ="0">//悬浮按钮
...
var w = floaty.rawWindow(
  ...
  ui.run(function(){
    ui.inflate( 
      ...
      <pref-checkbox id= "stuckflag" text = "卡死检测" layout_weight = "1"/>
      <pref-checkbox id= "loseflag" text = "任务失败检测" layout_weight = "1"/>
  ...
  switch(w.r1.indexOfChild(w.findView(w.r1.getCheckedRadioButtonId()))){
    case 0:
      ...
      events.on("dailyexec",function(i){
        if(i>dailyarr.length -1){toastLog("日常完成")}
        else{
        switch(i){
          case 0:
            if(dailyarr[0]){
          ...
          case 3:
            if(dailyarr[3][0]){
              storage.put("Recoverflag",true);
              ngines.execScriptFile(scriptarr[3])
            }else if(dailyarr[3][1]){
              storage.put("Recoverflag",true);
              ngines.execScriptFile(scriptarr[4])
...
w.b3.on("click",function(){
  ...
  switch(w.r3.indexOfChild(w.findView(w.r3.getCheckedRadioButtonId()))){
    ...
    break;
    case 5:
      engines.execScriptFile("./autoprc.js")
    break;
  }
...
setInterval(() => {
    if (isHorizontal != isHorizontalScreen()) {
        ...
        if(isHorizontal){
            w.setPosition(device.height/2 - 900,100)
            win_1.setPosition(0,50)//悬浮按钮定位
        }else{
            w.setPosition(100,device.height/2 - 900)
